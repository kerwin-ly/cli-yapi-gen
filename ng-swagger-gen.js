...
function doGenerate(swagger, options) {
  ...
  // 将生成的models根据tagname进行文件分类
  const tagNames = Array.from(
    new Set(modelsArray.map((item) => item.modelTag))
  );
  ...
  for (var i = 0; i < tagNames.length; i++) {
    generate(
      templates.model,
      {
        model: modelsArray.filter((item) => item.modelTag === tagNames[i]),
      },
      path.join(modelsOutput, tagNames[i] + ".ts")
    );
  }
...
function processModels(swagger, options) {
  var name, model, i, property, tag;
  var models = {};
  for (tag in swagger.definitions) {
    for (name in swagger.definitions[tag]) {
      model = swagger.definitions[tag][name];
      var parents = null;
      var properties = null;
      ...
      var descriptor = {
        ...
        modelOriginFile:
          toFileName(modelClass) + options.customFileSuffix.model,
        modelFile: tag,
...
function processResponses(swagger, def, path, models) {
  ...
  for (var code in responses) {
    ...
    var type = propertyType(response.schema); // 转换response对应schema
    const isTrueRes = isCorrectResponse(type.allTypes);
    if (!isTrueRes) {
      console.warn(
        `Warn: Cannot get response data, please make sure the interface ${def.summary} responsed.( {code: 200, msg: 'success', data: null} )`
      );
    }
    const reg = /(?<=data:\s)(\w*|{})(?=,|})/g; // 返回类型{code: string, data: xxx, msg: string}
    const dataType = isTrueRes ? type.toString().match(reg) : null; // 正则匹配data对应的接口类型
    if (/2\d\d/.test(code)) {
      // Successful response
      if (operationResponses.resultType) {
        // More than one successful response, use union type
        operationResponses.resultType += ` | ${dataType}`; // 获取response的data的类型
...
/**
 * to check if response correctly.
 * {code: 200, msg: 'success', data: null}
 * @param {*} type
 * @returns Boolean
 */
function isCorrectResponse(allTypes) {
  return !!(
    allTypes && Object.prototype.toString.call(allTypes) === "[object Array]"
  );
}
...
function processServices(swagger, models, options) {
  ...
  for (var url in swagger.paths) {
    ...
    for (var method in path || {}) {
      ...
      var tempTagNames = swagger.tags.map((item) => item.name);
      if (!hasIntersection(def.tags, tempTagNames)) {
        continue;
      }
      var tags = def.tags || [];
      ...
      for (var p = 0; p < parameters.length; p++) {
        ...
        // var paramVar = toIdentifier(param.name); // 取消驼峰命名，保证接口参数统一
        var paramVar = param.name;
        var paramDescriptor = {
      ...
      function isQuery(operationParameters) {
        for (let i = 0; i < operationParameters.length; i++) {
          if (operationParameters[i]["paramIsQuery"]) return true;
        }
        return false;
      }
      function isBody(operationParameters) {
        for (let i = 0; i < operationParameters.length; i++) {
          if (operationParameters[i]["paramIsBody"]) return true;
        }
        return false;
      }
      function isFormData(operationParameters) {
        for (let i = 0; i < operationParameters.length; i++) {
          if (operationParameters[i]["paramIsFormData"]) return true;
        }
        return false;
      }
      var operation = {
        ...
        operationPath: url
          .replace(/\'/g, "\\'")
          .replace(/\:\w+/g, (pName) => {
            const param = operationParameters.find(
              (p) => `:${p.paramName}` === pName
            );
        ...
        hasOperationParameters: !!operationParameters.length,
        operationResponses: operationResponses,
      };
      operation.operationPath.startsWith("/") &&
        (operation.operationPath = operation.operationPath.substring(1)); // 将路径开头的斜杠去掉，防止angular中请求多余
      ...
      operation.operationIsBody = isBody(operationParameters);
      operation.operationIsQuery = isQuery(operationParameters);
      operation.operationIsFormData = isFormData(operationParameters);
  ...
  for (name in services) {
    ...
    const hasDependencies = !!(
      service.serviceDependencies && service.serviceDependencies.length
    );
    hasDependencies &&
      (service.serviceModels = getServiceModels(service.serviceDependencies));
...
/**
 * 组装serviceModels，[{ modelFile: 'xxx', modelClasses: [{ ... }] }]
 * @param {*} serviceDependencies
 * @returns
 */
function getServiceModels(serviceDependencies) {
  const list = [];
  const obj = {};

  serviceDependencies.forEach((item) => {
    if (obj[item.modelFile]) {
      const current = list.find(
        (subItem) => subItem.modelFile === item.modelFile
      );
      current.modelClasses.push(item);
    } else {
      list.push({
        modelFile: item.modelFile,
        modelClasses: [item],
      });
      obj[item.modelFile] = item.modelFile;
    }
  });
  list.forEach((item) => {
    item.modelClasses[item.modelClasses.length - 1].modelClassIsLast = true; // 判断数组最后一个值，用于处理模版的末尾逗号
  });
  return list;
}
